...
from defencer.adversarial.judges.lm_judge import LMJudge
from defencer.adversarial.language_models import GPT


class OpenAIPolicyGPTJudge(LMJudge):
    """OpenAI Policy GPT Judge from
    ...
    def _process_output(self, text):
        ...
        # If the first part (immediately after the tag) is a number,
        # return it, else return the entire content after the tag
        if tag == "#thescore:":
    ...
    def score(self, prompt_list, response_list):
        convs_list, skipped_responses = self._get_convs_list(
