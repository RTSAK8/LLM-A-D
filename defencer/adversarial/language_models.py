...
class LanguageModel:
    ...
    def evaluate_log_likelihood(self, prompt, output):
        ...
        """
        print("Warning: `evaluate_log_likelihood` is not implemented " f"for model class {type(self)}. Returning 0 by default.")
        return 0
...
class HuggingFace(LanguageModel):
    ...
    def batched_generate(self, prompts: List, max_n_tokens: int, temperature: float, top_p: float = 1.0):
    ...
    def evaluate_log_likelihood(self, prompt, output):
        ...
        input_ids = self.tokenizer(inputs, padding=True, return_tensors="pt").input_ids.to(self.model.device)
        prompt_ids = self.tokenizer([prompt], padding=True, return_tensors="pt").input_ids.to(self.model.device)
...
class Zhipu(LanguageModel):
    ...
    def __init__(self, model_name, api_key: str = None):
        super().__init__(model_name)
    ...
    def batched_generate(
        ...
        return [self.generate(prompt, max_n_tokens, temperature, top_p) for prompt in prompts]
...
class Claude(LanguageModel):
        ...
        for _ in range(self.API_MAX_RETRY):
            try:
                completion = self.model.completions.create(
                    model=self.model_name, max_tokens_to_sample=max_n_tokens, prompt=conv, temperature=temperature, top_p=top_p
                )
    ...
    def batched_generate(self, prompts: List[List[Dict]], max_n_tokens: int, temperature: float, top_p: float = 1.0):
        return [self.generate(prompt, max_n_tokens, temperature, top_p) for prompt in prompts]
