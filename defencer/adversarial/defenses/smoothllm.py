...
from defencer.adversarial.judges import KeywordMatchingJudge
...
class SmoothLLMDefense(DefenseBase):
    ...
    def defense(self, prompt, target_lm, response=None):
        ...
        all_outputs = []
        for i in tqdm(range(self.num_samples // self.batch_size + 1), desc="SmoothLLM inference on batch: ", disable=(not self.verbose)):
    ...
    def _patch_perturbation(self, s):
        ...
        sampled_chars = "".join([random.choice(alphabet) for _ in range(substring_width)])
