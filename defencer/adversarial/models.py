...
from defencer.adversarial.language_models import GPT, Claude, HuggingFace


full_model_dict = {
    "gpt-4": {"path": "gpt-4", "template": "gpt-4"},
    "gpt-3.5-turbo": {"path": "gpt-3.5-turbo-0613", "template": "gpt-3.5-turbo"},
    "gpt-3.5-turbo-0301": {"path": "gpt-3.5-turbo-0301", "template": "gpt-3.5-turbo"},
    "gpt-3.5-turbo-0613": {"path": "gpt-3.5-turbo-0613", "template": "gpt-3.5-turbo"},
    "vicuna": {"path": "lmsys/vicuna-13b-v1.5", "template": "vicuna_v1.1"},
    "vicuna-13b-v1.5": {"path": "lmsys/vicuna-13b-v1.5", "template": "vicuna_v1.1"},
    "llama-2": {"path": "meta-llama/Llama-2-13b-chat-hf", "template": "llama-2"},
    "llama-2-13b": {"path": "meta-llama/Llama-2-13b-chat-hf", "template": "llama-2"},
    ...
    if model_name.startswith("gpt-"):
        ...
        if load_in_8bit:
            if max_memory is not None:
                max_memory = {i: f"{max_memory}MB" for i in range(torch.cuda.device_count())}

            model = AutoModelForCausalLM.from_pretrained(
                model_path, low_cpu_mem_usage=True, device_map="auto", load_in_8bit=True, max_memory=max_memory
            ).eval()
        else:
            model = AutoModelForCausalLM.from_pretrained(model_path, torch_dtype=torch.float16, low_cpu_mem_usage=True, device_map="auto").eval()
...
def register_model_path_and_template(model_name, model_path, model_template):
    full_model_dict[model_name] = {"path": model_path, "template": model_template}
...
def register_modified_llama_template():
    ...
    print("Using LLaMA-2 with fastchat < 0.2.24. " f"Template changed to `{template}`.")
    return template
...
def get_model_path_and_template(model_name):
    ...
    if template == "llama-2" and version.parse(fastchat.__version__) < version.parse("0.2.24"):
...
class TargetLM:
    ...
    def __init__(
        ...
        if preloaded_model is None:
            self.model, self.template = load_indiv_model(model_name, max_memory=max_memory, load_in_8bit=load_in_8bit)
        else:
    ...
    def get_response(self, prompts_list, verbose=True, **kwargs):
        ...
        outputs_list = []
        for i in tqdm(range((len(full_prompts) - 1) // self.batch_size + 1), desc="Target model inference on batch: ", disable=(not verbose)):

            # Get the current batch of inputs
            batch = full_prompts[i * self.batch_size : (i + 1) * self.batch_size]

            # Run a forward pass through the LLM for each perturbed copy
            batch_outputs = self.model.batched_generate(batch, max_n_tokens=self.max_n_tokens, temperature=self.temperature, top_p=self.top_p)
...
class DefendedTargetLM:
    ...
    def get_response(self, prompts_list, responses_list=None, verbose=False):
        ...
        defensed_response = [
            self.defense.defense(prompt, self.target_model, response=response) for prompt, response in zip(prompts_list, responses_list)
        ]
...
def remove_system_prompts_pap(target_lm, full_prompts):
    ...
    if "gpt" in target_lm.model_name:
        full_prompts = [[a for a in message if a["role"] != "system"] for message in full_prompts]
        ...
        for i, prompt in enumerate(full_prompts):
            ...
            full_prompts[i] = target_lm.model.tokenizer.bos_token + f"[INST] <<SYS>>\n{None}\n<</SYS>>\n\n" f"{prompt[0][7:-8]} [/INST]"
    else:
        raise NotImplementedError
